...
public class ThreadRaceGame {
    ...
    public static void main(String[] args) {
        ...
        //레이서 객체 3개 생성
        Thread racer1 = new Thread(new Racer("토끼"), "[토끼스레드]");
        Thread racer2 = new Thread(new Racer("사자"), "[사자스레드]");
        Thread racer3 = new Thread(new Racer("공룡"), "[공룡스레드]");
    ...
    public static synchronized void printRaceProgress(String name, int position) {
        ...
        String icon = null;
        switch (name){
            case "[사자스레드]" : icon = "🦁"; break;
            case "[토끼스레드]" : icon = "🐰"; break;
            case "[공룡스레드]" : icon = "🦖"; break;
        }
